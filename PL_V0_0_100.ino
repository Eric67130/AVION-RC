...
const char *FIRMWARE_VERSION = "v0.0.100";
...
#define BAT_ADC_PIN 1  // Pin D0 (GPIO1) libre
...
void applyGimbal(int panPWM, int tiltPWM) {
  pwm.writeMicroseconds(GIMBAL_PAN_CHANNEL, panPWM);
  pwm.writeMicroseconds(GIMBAL_TILT_CHANNEL, tiltPWM);
}
...
void TaskControlFlight(void *pvParameters) {
  ...
  for (;;) {
    ...
    if (xSemaphoreTake(mutex, portMAX_DELAY)) {
      ...
      pwm.writeMicroseconds(0, targetL);
      pwm.writeMicroseconds(1, targetR);
      pwm.writeMicroseconds(2, targetElev);
      pwm.writeMicroseconds(3, (int)currentFlaps);
      pwm.writeMicroseconds(6, (int)currentFlaps);
      pwm.writeMicroseconds(7, targetRud);
      pwm.writeMicroseconds(4, targetAil1);
      pwm.writeMicroseconds(5, targetAil2);
...
void debugControlData(const ControlData &data) {
  ...
  Serial.printf("AILERONS| Ail1: %4d µs | Ail2: %4d µs\n", telem.ail1PWM, telem.ail2PWM);
        ...
        if (rxData.aircraftId == MY_AIRCRAFT_ID) {
          ...
          if (packetCount % 5 == 0) {
            if (xSemaphoreTake(mutex, portMAX_DELAY)) {
              ...
              //telem.batteryVoltage = (liveEscData.voltage > 1.0) ? liveEscData.voltage : (analogRead(BAT_ADC_PIN) / 4095.0) * 3.3 * 5.75;
